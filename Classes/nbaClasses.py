...
# TODO currently working on adding advanced stats. Advanced stats for this season working in nbaScraper.py
    ...
    def getStats(self, season=None):
        if season is not None:  # getting a season, not the whole career
            ...
            for year in seasons:  # appends data frame of each season to careerStats
                stats.append(Season(stats[0].gameData[totalGames:totalGames + seasons[year]]))  # split seasons up
...
class Season:
    ...
    def getPTS(self, team=None):
        if team is None:
            totalPoints = sum(filter(None, self.gameData['PTS']))
        else:
            team = getTeamAbbreviation(team)
            totalPoints = 0
    ...
    def getREB(self, team=None):
        if team is None:
            totalREB = sum(filter(None, self.gameData['REB']))
        else:
            team = getTeamAbbreviation(team)
            totalREB = 0
            for game in self.gameData.iterrows():
    ...
    def getAST(self, team=None):
        if team is None:
            totalAST = sum(filter(None, self.gameData['AST']))
        else:
            team = getTeamAbbreviation(team)
            totalAST = 0
            for game in self.gameData.iterrows():
