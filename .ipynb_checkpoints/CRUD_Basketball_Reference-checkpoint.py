    ...
    for season in range(start_year, end_year):
        ...
        for game_index in range(len(year)):
            exists = db.session.query(season_model.schedule).filter_by(date=year['DATE'][game_index],
                                                                       ...
                                                                       visitor_id=year['VISITOR']).first() is not None  # checks if team is already in table
            if exists:
                ...
                db.session.add(season_model.schedule(date=year['DATE'][game_index],
                                                     ...
                                                     visitor_pts=helpers.int_cast(year['VISITOR_PTS'][game_index])))  # add new season to scheduled commits
        print(f"Adding schedule for {season} Progress: {((season-start_year) / (end_year-start_year)) * 100}%")
...
def create_roster(season_start=None, season_end=datetime.datetime.today().year):
    """
    ...
    for season in range(season_start, season_end + 1):
        ...
        for t in team_list:
            ...
            if data is not None:
                ...
                for player_index in range(len(data['PLAYER'])):
                    if db.session.query(to_id.player).filter_by(name=data["PLAYER"][player_index]).first() is None:  # check if player exists in player_name_to_id and adds if not
                        db.session.add(to_id.player(name=data["PLAYER"][player_index]))
                        db.session.commit()
        ...
        print(f"Adding roster for {season} Progress: {((season-season_start) / (season_end-season_start)) * 100}%")
...
def create_standings(starting_date=None):  # created through 1970-10-13
    ...
    if starting_date is not None:
        dates = db.session.query(season_model.schedule.date).filter(season_model.schedule.date > starting_date, season_model.schedule.date not in db.session.query(season_model.standings.date)).order_by(season_model.schedule.date).distinct().all()
    else:
        dates = db.session.query(season_model.schedule.date).filter(season_model.schedule.date not in db.session.query(season_model.standings.date)).order_by(season_model.schedule.date).distinct().all()
    ...
    for index, date in enumerate(dates):
            ...
            for i in range(len(standings[key]['TEAM'])):  # index 0 is always a division
                ...
                try:
                    exists = db.session.query(season_model.standings).filter_by(team_id=team.id,
                                                                                locale_id=current_division.id if current_division is not None else helpers.get_locale(key).id,
        ...
        print(f"Adding standings for {date} Progress: {index/len(dates)*100}%")
...
def create_shot_chart(game_id=None):  # created shot charts from game 1 through game 29941
    ...
    if game_id is None:
        ...
        games = db.session.query(func.max(season_model.schedule.game_id)).filter(season_model.schedule.game_id >= int(game_id)).order_by(season_model.schedule.game_id.asc()).all()
    ...
    for index, g in enumerate(games):
        ...
        print(f'Adding game: {team1.full_name} vs. {team2.full_name} - {game.date} Progress: {index/len(games)*100}%')
        db.session.commit()  # commit each game in case of error late in process
        time.sleep(1)  # avoid disconnection from host
...
def create_team_per_x(start_season=1951, format="PER_GAME"):
    ...
    """
    season_list = db.session.query(season_model.schedule).distinct(season_model.schedule.season_id).filter(season_model.schedule.season_id > start_season)
    team_list = helpers.get_all_teams()
...
def create_opponent_per_x(update_from=1949, format="TOTALS"):
    ...
    Data starts in 1950
    """
    season_list = db.session.query(season_model.schedule).distinct(season_model.schedule.season_id).filter(season_model.schedule.season_id > update_from)
    team_list = helpers.get_all_teams()
    for season in season_list:
        for team in team_list:
            time.sleep(random.randint(50, 250) / 100)
...
def create_roster_per_x(format_id="PER_GAME", update_from=1949, playoffs=False, team_list=None):
    ...
    season_list = db.session.query(season_model.schedule).distinct(season_model.schedule.season_id).filter_by(season_model.schedule.season_id > update_from)
    for season in season_list:
        for team in team_list:
            time.sleep(random.randint(50, 250) / 100)
            ...
            try:
                ...
                for player_index in range(len(roster_per_x['PLAYER'])):
                    ...
                    if exists:
                        ...
                        db.session.add(player_model.roster_per_x(age=roster_per_x['AGE'][player_index],
                                                                 ...
                                                                 player_id=player_id,
                                                                 position=to_id.position_converter(roster_per_x['POS'][player_index]),
                ...
                print(error)
                print('No per_game stats for ' + str(team) + ' for ' + str(season))
                continue  # some years only have data but don't have any of the other stat types
        db.session.commit()  # commit season by season
        time.sleep(2)  # sleep to avoid Connection Error when drawing too many results


def create_box_score_per_x(period="GAME", game_id=None):
    """
                        ...
                        if exists:
                            ...
                            db.session.add(box_score_model.box_score_per_x(game_id=helpers.get_game(date=date,
                                                                           ...
                                                                           player_id=helpers.get_player(
                                                                               box_score[team]['PLAYER'][player_index]).id,
                                                                           ...
                                                                           plus_minus=box_score[team]['+/-'][player_index],
                                                                           period=period
                                                                           ))
        else:
            continue
        db.session.commit()  # commit full games
        helpers.int_cast(f'Committed {home} vs {visitor}, {date}, q1')


def create_box_score_advanced(date=None):
    """
    ...
    if date is None:
        games = db.session.query(season_model.schedule).order_by(season_model.schedule.date.asc()).all()

    else:
        games = db.session.query(season_model.schedule).filter(season_model.schedule.date > str(date)).order_by(
            season_model.schedule.game_id.asc()).all()
    ...
    for game in games:
        ...
        if box_score.keys() is not None:
            for team in box_score.keys():
                for player_index in range(len(box_score[team]['PLAYER'])):
                    if box_score[team]['MP'][player_index] == 'Did Not Play' or box_score[team]['MP'][player_index] == 'Did Not Dress':  # skip players who didn't play
                        continue
        ...
        helpers.int_cast(f'Committed {helpers.get_team(game.home_id).full_name} vs {helpers.get_team(game.visitor_id).full_name}, {game.date}, advanced')
...
def create_player_game_logs(name=None, playoffs=False):
    ...
    if name is None:  # SELECTS players who played in given season
        player_list = db.session.query(team_model.roster.player_id).filter(team_model.roster.player_id not in db.session.query(player_model.game_log.player_id), player_model.game_log.playoffs == playoffs).distinct().all()

    elif type(name) is str:  # converts type of name if user inputs one name as a string
        player_list = [helpers.get_player(name)]

    else:
        raise TypeError("Name should be a string containing a players name or id or None to get the game logs for all players")

    for p in player_list:
        time.sleep(random.randint(50, 250) / 100)
        career = helpers.get_career_seasons(p.name)  # find out which seasons player was in the league to cut down on wasted queries
        for season in range(career[0], career[1] + 1):
            try:
                game_logs = players.get_game_logs(_name=p.full_name, year=season, playoffs=playoffs)  # getting attribute error that NoneType has no attribute replace for everything
                time.sleep(2)  # wait between 1 and 5 seconds to avoid attempts to disconnect
            except (UnboundLocalError, IndexError, AttributeError):  # Unbound error when player did not play in the season or Index Error/Attribute error in package for NoneType
                    ...
                    if exists:
                        ...
                        db.session.add(
                            player_model.game_log(game_id=helpers.get_game(date=game_logs['DATE'][game_index],
                                                  ...
                                                  blk=game_logs.get('BLK', [None] * (game_index + 1))[game_index],  # not recorded in old games, returns None instead with dict.get()
                                                  drb=game_logs.get('DRB', [None] * (game_index + 1))[game_index],  # not recorded in old games
                                                  fg=game_logs['FG'][game_index],
                                                  fga=game_logs['FGA'][game_index],
                                                  ft=game_logs['FT'][game_index],
                                                  fta=game_logs['FTA'][game_index],
                                                  game_score=game_logs.get("GAME_SCORE", [None] * (game_index + 1))[game_index],  # not recorded in old games
                                                  game_started=game_logs['GS'][game_index],
                                                  minutes_played=game_logs['MP'][game_index],
                                                  orb=game_logs.get('ORB', [None] * (game_index + 1))[game_index],  # not recorded in old games
                                                  ...
                                                  turnovers=game_logs.get('TOV', [None] * (game_index + 1))[game_index],  # not recorded in old games
                                                  trb=game_logs['TRB'][game_index]
                                                  ))

        db.session.commit()  # commit player by player
        print(f"Committed {p.name}")


def create_player_per_x(format="PER_GAME", playoffs=False, career=False):
    """
    ...
    """
    format = helpers.get_format(format)
    player_list = db.session.query(to_id.player).filter(to_id.player.id not in db.session.query(player_model.player_per_x).filter(player_model.player_per_x.playoffs == playoffs, player_model.player_per_x.career == career, player_model.player_per_x.format_id == format.id)).all()
    failures = 0
    for p in player_list:
        time.sleep(random.randint(50, 250) / 100)
        try:
            ...
            if exists:
                ...
                try:
                    db.session.add(player_model.player_per_x(age=stats['AGE'][season_index],
                                                             ...
                                                             blk=stats.get('BLK')[season_index] if stats['BLK'] is not None else None,
                                                             drb=stats.get('DRB')[season_index] if stats['DRB'] is not None else None,
                                                             ...
                                                             mp=stats.get('MP')[season_index],
                                                             orb=stats.get('ORB')[season_index] if stats.get("ORB") is not None else None,
                                                             pf=stats.get('PF')[season_index],
                                                             pts=stats.get('PTS')[season_index],
                                                             season_id=stats.get('SEASON')[season_index],
                                                             stl=stats.get('STL')[season_index] if stats.get("STL") is not None else None,
                                                             team_id=helpers.get_team(stats.get('TEAM')[season_index]).id,
                                                             tov=stats.get('TOV')[season_index] if stats.get("TOV") is not None else None,
                    ...
                    failures += 1
                    Warning(f"Failure {failures} for {p.name} in season {season_index}, format: {format.format}. Stacktrace: {e}")
                    continue
        try:
...
def create_player_advanced(playoffs=False, career=False):
    ...
    player_list = db.session.query(to_id.player.id).filter(to_id.player.id not in db.session.query(player_model.player_advanced.player_id)).all()
    for p in player_list:
        time.sleep(random.randint(50, 250) / 100)
        try:
...
def create_pbp(game_id=None):
    ...
    """
    if game_id is None:
        games = db.session.query(season_model.schedule.game_id).filter(season_model.schedule.game_id not in db.session.query(team_model.play_by_play).distinct()).distinct().all()
        ...
        games = db.session.query(season_model.schedule).filter(season_model.schedule.game_id > game_id, season_model.schedule.game_id not in db.session.query(team_model.play_by_play.game_id)).order_by(asc(season_model.schedule.date)).distinct().all()
    ...
    for game in games:
        time.sleep(random.randint(50, 250) / 100)
        try:
            pbp_stats = pbp.get_pbp(date=game.date,
                                    team1=helpers.get_team(game.home_id).abbreviation,
                                    team2=helpers.get_team(game.visitor_id).abbrevistion)

        except AttributeError:  # package throws AttributeError if there is no data
            continue

        for play_index in range(len(pbp_stats['TIME_REMAINING'])):
            play = helpers.play_breakdown(pbp_stats.iloc[[play_index]])

            exists = db.session.query(team_model.play_by_play).filter_by(quarter=pbp_stats['QUARTER'][play_index],
                                                                         game_id=game.id,
                                                                         time_remaining=pbp_stats['TIME_REMAINING'][
                                                                             play_index],
                                                                         action=play['ACTION'],
                                                                         acting_team_id=helpers.get_team(play['ACTING TEAM']).id,  # get ID from full name
            ...
            if exists:
                ...
                db.session.add(team_model.play_by_play(quarter=pbp_stats['QUARTER'][play_index],
                                                       game_id=game.id,
                                                       time_remaining=pbp_stats['TIME_REMAINING'][play_index],
                                                       action=play['ACTION'],
                                                       acting_team_id=helpers.get_team(pbp_stats['ACTING TEAM'][play_index]).id,
                                                       acting_team_score=play['ACTING TEAM SCORE'],
                                                       opponent_team_id=helpers.get_team(pbp_stats['OPPONENT']).id,
                                                       opponent_score=play['OPPONENT SCORE']))
...
def update_box_score_per_x(period, playoffs=False):
    game_id = db.session.query(func.max(box_score_model.box_score_per_x.game_id)).filter(box_score_model.box_score_per_x.format_id == helpers.get_format(period), box_score_model.box_score_per_x.playoffs == playoffs).first()[0]
...
def update_roster_per_x(format="TOTALS", playoffs=False):
    ...
    playoffs = True, False
    """
    max_season = db.session.query(func.max(player_model.roster_per_x.season_id)).filter(player_model.roster_per_x.format_id == helpers.get_format(format).id, player_model.roster_per_x.playoffs == playoffs).first()[0]
...
def update_opponent_per_x(format):
    ...
    """
    format = helpers.get_format(format)
    max_season = db.session.query(func.max(team_model.opponent_per_x.season_id)).filter(team_model.opponent_per_x.format_id == format.id).first()[0]
...
def update_shot_chart():
    ...
    """
    for i in range(1000):  # keep running after error
        try:
...
def update_standings(max_date=None):
    ...
    if max_date is None:
        max_date = str(db.session.query(func.max(season_model.standings.date)).first()[0])#.split(' ')[0]  # switch to date instead of year?
...
def create_per_all(shuffle=False):
    ...
    for f in formats:
        for p in playoffs:
            for c in career:
                create_player_per_x(format_id=f, playoffs=p, career=c)
...
def starting_build():
    ...
    create_roster()  # build create_player_name_to_id while creating roster
...
if __name__ == '__main__':
    ...
    # TODO refill tables in postgres db- list tables in starting build to consolidate functions to build tables
    # TODO add way to manage career length in player_per_x. It's currently a string like '7 seasons' but the season_id field is an int
